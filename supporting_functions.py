...
def create_output_images(Rover):
      ...
      if rock_world_pos[0].any():
            ...
            for idx in range(len(Rover.samples_pos[0])):
                  ...
                  if np.min(rock_sample_dists) < 3:
                        ...
                        Rock_num+=1
      Rover.mean_dir=np.mean(Rover.nav_angles*180/math.pi)
      ...
      cv2.putText(map_add,"Fidelity: "+str(fidelity)+'%', (0, 40),
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      cv2.putText(map_add,"Rocks: "+str(Rock_num), (0, 55),
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      cv2.putText(map_add,"Direction: "+str(Rover.mean_dir), (0, 70),
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      cv2.putText(map_add,"Loop No: "+ (Rover.mode), (80,10),
                    cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      ...
      pil_img = Image.fromarray(Rover.warped_image.astype(np.uint8))
      buff = BytesIO()
      pil_img.save(buff, format="JPEG")
